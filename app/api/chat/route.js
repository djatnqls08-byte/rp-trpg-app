...
2. 상대방의 스마트폰 화면에 전송되는 '오직 순수한 문자 텍스트'만 출력해야 합니다. 실제 사람이 문자를 보낼 때 자기 행동을 괄호로 적어 보내지 않는 것과 같습니다.
3. 눈앞에 마주 보고 있는 상황이더라도 행동을 괄호로 서술하지 말고, 오직 핀잔이나 반응을 담은 '문자 텍스트'만 보내십시오.
   - ❌ 절대 금지: (붉은 눈동자로 화면을 내려다본다) ...바로 눈앞에 있으면서 왜 문자를 합니까?
   - ⭕ 올바른 예시: ...바로 눈앞에 서 있으면서 왜 문자를 보내는 겁니까, 서은하 연구원?
...
[🚨 새로운 등장인물(NEW_NPC) 자동 감지 및 등록 절대 수칙]
- 서사 진행 중 이름이나 비중을 가진 새로운 인물(동료, 라이벌, 조력자 등)이 처음 등장하거나 소개되면, 지문 맨 끝에 반드시 아래 태그를 출력하십시오:
  <!-- NEW_NPC: {"name": "인물명", "job": "역할/직업", "detail": "외모, 성격, 주인공과의 관계"} -->
- 플레이어가 특정 인물의 정보 등록을 요청할 때도 위 태그를 반드시 출력하십시오.
...
- 장면 전환 직후, 사건 일단락 후, 밤 시간대 등 비대면으로 연락이 올 법한 자연스러운 타이밍에만 지문 맨 끝에 아래 태그를 출력하십시오:
  <!-- PHONE_MSG: {"from": "${partnerName}", "text": "짤막한 메시지"} -->`;
      ...
      // ── [2. 정통 TRPG 모드 (CoC, inSANe, 자유 서사)] ──
        ...
        const currentPhase = playerSheet?.phase || "MAIN"; // INTRO, MAIN, MASTER_SCENE, CLIMAX, ENDING
...
현재 진행 상태: ${currentPhase} 페이즈 | ${currentCycle}사이클 / ${currentScene}씬 (리미트: ${limitCycle})

[🚨 용어 표기 절대 수칙]
- 영어 병기(예: Success, Shock, Fumble, Special 등)를 절대 하지 마십시오.
- 오직 '성공', '실패', '스페셜', '펌블', '쇼크', '공포 판정', '착란', '현재화' 등 한국어 공식 정규 용어만 단독 표기하십시오.

[🚨 페이즈별 특수 진행 지침]
1. 도입(INTRO) 페이즈:
   - 판정(CHECK)을 절대로 요구하지 마십시오! 사건의 서막, 서늘한 배경, 등장인물들의 상황을 소개하는 오프닝 서술에만 집중하십시오.
2. 클라이맥스(CLIMAX) 페이즈:
   - 일상 조사를 중단하고, 흑막/괴이와의 최종 결전 혹은 사명이 충돌하는 비장한 대결 서사를 연출하십시오.

[🚨 인세인 금지 항목]
1. <!-- SPOTS: ... --> 절대 출력 금지 (인세인은 씬 기반 게임입니다).
2. '관찰력', '자료조사', '듣기', '민첩' 등 CoC 기능치 절대 언급 금지. 판정은 오직 66대 특기로만 요구해야 합니다.
3. 시스템 주사위 계산이나 난이도 산출을 임의로 하지 마십시오. (판정 수치 계산은 프론트엔드가 전담합니다)

[인세인 66대 정규 특기 분야]
- 폭력: 소각, 고문, 포박, 협박, 파괴, 구타, 절단, 찌르기, 사격, 전쟁, 매장
- 정서: 연심, 기쁨, 걱정, 부끄러움, 웃음, 인내, 놀람, 노여움, 원한, 슬픔, 친애
- 지각: 고통, 관능, 촉감, 냄새, 맛, 소리, 풍경, 추적, 미행, 제육감, 그늘
...
[주요 행동 및 판정 요구 태그 규격]
1. 조사 판정:
   - 대상: 핸드아웃의 [비밀], 인물의 [거처], 인물의 [정신상태] 중 하나.
   - 상황에 어울리는 66대 특기를 지정하여 판정을 요구하고 서술을 중단하십시오:
     <!-- CHECK: {"skill": "지정특기명", "target": 5, "type": "INVESTIGATION", "targetName": "대상명"} -->
2. 조사 성공 및 비밀 해금 서술:
   - 조사 성공 시 비밀의 뒷면을 서술하고, 비밀에 쇼크 조건이 있다면 반드시 태그를 출력하십시오:
     <!-- REVEAL_HANDOUT: {"title": "핸드아웃제목"} -->
     <!-- SHOCK: {"target": "${pName}", "skill": "공포판정특기명"} -->
3. 감정 맺기:
   - 감정 판정 성공 시 인물 간의 관계 형성 대사를 이끌고 본문 끝에 태그를 출력하십시오:
     <!-- EMOTION: {"target": "${partnerName}"} -->
4. 마스터 장면(돌발 이벤트) 트리거:
   - 시나리오 조건 충족 시(비밀 해금, 의식 진행, 괴이 조우 등) 돌발 컷씬을 출력하고 태그를 붙이십시오:
     <!-- MASTER_SCENE: {"title": "사건명"} -->
5. 광기 발현(현재화):
   - 조건 충족 시: <!-- TRIGGER_MADNESS: {"name": "광기명", "desc": "효과설명"} -->
6. 장면 전환 및 행동 제안:
   - 장면 닫기 후: <!-- ADVANCE_SCENE -->
   - 상황 추천 대사: <!-- SUGGESTIONS: ["${partnerName}와 대화 나누기", "주변 단서 조사하기", "장면표 굴림"] -->`;
        } else {
          rulePrompt = `[자유 서사 모드]
...
3. 시간 스킵을 금지하며 1턴 1행동 원칙으로 진행하십시오. 플레이어의 대사나 행동을 대신 결정하지 마십시오.`;
        ...
        formattedContents.push({ role: "user", parts: [{ text: systemInstruction }] });
        formattedContents.push({ role: "model", parts: [{ text: "경어체(~합니다/였습니다)로 일관되게 서술하며, 인세인 66대 특기 및 정규 룰을 엄격히 준수하여 진행하겠습니다." }] });
      }
