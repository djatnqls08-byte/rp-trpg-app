...
export async function POST(req) {
  try {
    ...
    if (isScenarioGeneration) {
      ...
      [세계관 및 관계성 절대 규칙]
      1. 맹목적 추종, 얀데레, 통제권 상실, 폭력적 소유욕(너는 내 것 등), 납치/감금 설정은 어떠한 경우에도 엄격히 배제합니다.
      2. 인물들은 서로에게 '깊은 신뢰와 정서적 유대'를 가지며, 적당한 미학과 거리감을 유지한 성숙한 관계를 맺습니다.
      3. 반환값은 반드시 마크다운(\`\`\`json ... \`\`\`)을 제외한 순수한 JSON 객체 형식이어야만 합니다.
      `;
    } else {
      // 2. [인게임 채팅 & 롤플레잉(RP) 모드]
      systemInstruction = `
