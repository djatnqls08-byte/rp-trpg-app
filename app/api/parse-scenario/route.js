...
export async function POST(req) {
  try {
    ...
    // 🌟 [수정 완료] 문서가 '캐릭터 시트' 단독일 때도 완벽하게 추출해내는 만능 파서 규칙 추가
    const systemPrompt = `
당신은 TRPG 데이터 및 캐릭터 시트 파싱 전문가입니다.
제공된 문서(텍스트/이미지)가 '시나리오 원문'이든 '캐릭터 설정(시트) 전용'이든 상관없이, 포함된 정보를 아래의 JSON 포맷에 맞추어 완벽하게 추출하십시오.

[🚨 데이터 추출 절대 규칙]
1. 캐릭터 시트 최우선 추출: 문서에 주인공(PC)의 이름이나 설정이 있다면 pcName, pcJob, pcBackground 등에 상세히 추출하십시오.
2. 다수 NPC/KPC 필수 추출: 문서에 'KPC', 'NPC', 파트너 등 등장인물 정보가 있다면 단 1명이라도 반드시 "npcs" 배열 안에 객체로 분리하여 추출하십시오.
3. 강제 빈칸 규칙: 문서가 캐릭터 시트라서 시나리오 서막이나 진상 내용이 없다면 억지로 지어내지 말고 해당 필드는 강제로 "" (빈 문자열)로 둡니다.
      ...
      for (const modelName of FALLBACK_MODELS) {
        try {
          ...
          const result = await model.generateContent({
            ...
            generationConfig: { 
              temperature: 0.1, 
              responseMimeType: "application/json" // 🌟 무조건 JSON 형태로만 답변하도록 강제!
            } 
          });
