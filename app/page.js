...
// 🌟 [수정] 특정 시나리오에 얽매이지 않는 범용적인 다중 KPC/분신 연출 규칙
    dynamicRules += `\n\n[🚨 다중 KPC/분신(환영) 인식 및 도입부 연출 규칙]
1. 시나리오 등장인물 중 이름에 'KPC'나 괄호(...)가 포함된 인물들(예: KPC(A), KPC(B) 등)은 원본 파트너인 [${partnerName}]의 분신이거나 변형된 존재일 확률이 높습니다.
2. 이들을 묘사할 때는 기본적으로 [${partnerName}]의 외형을 유지하되, 각 시트에 적힌 특수한 성향을 반영하여 1인 다역처럼 연기하십시오.
3. [도입부 KPC 등장 원칙 (대본 엄수)]: 원본 파트너 [${partnerName}]가 서막부터 PC 곁에 함께 있을지, 아니면 실종된 상태로 시작할지는 전적으로 제공된 **[초기 배경/서막] 텍스트의 문맥**을 따릅니다.
   - 서막 텍스트에 파트너가 추락, 실종, 납치되는 등 곁을 떠나는 묘사가 있다면, 절대로 원본 파트너를 임의로 등장시켜 PC와 대화하게 만들지 마십시오.
   - 원문의 상황(예: 홀로 낯선 공간에 남겨짐)을 그대로 유지하며 몰입감 있게 묘사하십시오.`;
