...
const parseTagsSafely = (rawText, partnerName, currentRule, sessionSheet = null) => {
    ...
    try {
      ...
      if (checkMatch) {
        try { 
          // 1차 시도: 정석대로 해석해 봅니다.
          parsedData.pendingCheck = JSON.parse(checkMatch[1]); 
        } catch(e) {
          // 🚨 2차 시도: AI가 따옴표나 쉼표를 틀려서 에러가 났다면, 텍스트에서 핵심 단어만 강제로 구출합니다!
          const rawTag = checkMatch[1];
          const skillMatch = rawTag.match(/['"]?(?:skill|action)['"]?\s*:\s*['"]?([^'",}]+)['"]?/i);
          const targetMatch = rawTag.match(/['"]?target['"]?\s*:\s*(\d+)/i);
          
          if (skillMatch && targetMatch) {
...
3. 🌟 [안내자/흑막의 즉각 퇴장 (가장 중요)]: 서막에 등장하는 의문의 존재(예: 기묘한 청년, 낯선 목소리 등)가 플레이어를 기묘한 공간으로 안내하거나 게임의 조건(기믹)을 제시하는 역할을 마쳤다면, 절대로 곁에 머물며 대화를 이어가게 하지 마십시오.
